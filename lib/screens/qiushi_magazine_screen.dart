...
/// 《求是》杂志 — 年→期→文章
class QiushiMagazineScreen extends StatefulWidget {
  const QiushiMagazineScreen({super.key});
  @override
  State<QiushiMagazineScreen> createState() => _QiushiMagazineScreenState();
}

class _QiushiMagazineScreenState extends State<QiushiMagazineScreen>
    with SingleTickerProviderStateMixin {
  late TabController _tab;
  bool _loading = true;
  final _issues = <int, List<_Issue>>{};
  static const _years = [2026, 2025, 2024, 2023, 2022, 2021, 2020, 2019];

  static const _yearUrls = {
    2026: 'https://www.qstheory.cn/20251231/2d916da295774130ac2fb223fd208895/c.html',
    2025: 'https://www.qstheory.cn/20250101/f1fa7777504340aa8e6609e602eed4d1/c.html',
    2024: 'http://www.qstheory.cn/dukan/qs/2024-01/01/c_1130050739.htm',
    2023: 'http://www.qstheory.cn/dukan/qs/2023-01/01/c_1129248666.htm',
    2022: 'http://www.qstheory.cn/dukan/qs/2022-01/01/c_1128224917.htm',
    2021: 'http://www.qstheory.cn/dukan/qs/2021-01/01/c_1126937378.htm',
    2020: 'http://www.qstheory.cn/dukan/qs/2020-01/01/c_1125412076.htm',
    2019: 'http://www.qstheory.cn/dukan/qs/2019-01/01/c_1123932149.htm',
  };
    ...
    try {
      ...
      if (raw.isNotEmpty) {
        ...
        for (final y in _years) {
          _issues[y] = items.where((e) => e['source'] == 'qiushi_$y').map((e) => _Issue(
  ...
  Future<List<_Issue>> _scrapeYear(int year) async {
    ...
    // Find all plain text issue numbers: 《求是》2024年第1期
    final plainNums = <String>{};
    for (final m in RegExp(r'《求是》\s*' + year.toString() + r'年第(\d+)期(?:\s*</strong>)?').allMatches(html)) {
      plainNums.add(m.group(1)!);
    }

    // Find all linked issues with href
    final linkMap = <String, String>{};
    final linkRe = RegExp(r'<a[^>]*href="([^"]+?)"[^>]*>(?:<strong>)?\s*《求是》\s*' + year.toString() + r'年第(\d+)期');
    ...
    for (final num in plainNums) {
      ...
      list.add(_Issue(
        title: '${year}年第$num期',
        url: link ?? url, // fallback to year page if no direct link
        date: '$year-$num',
      ));
    }
  ...
  Future<void> _saveCache() async {
    try {
      ...
      all.removeWhere((e) => (e['source'] as String?)?.startsWith('qiushi_') == true);
      for (final y in _years) {
        for (final i in _issues[y]!) {
          all.add({'title': i.title, 'source': 'qiushi_$y', 'url': i.url, 'publishDate': i.date, 'category': '政治'});
        }
      }
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
          Container(width: 44, height: 44, decoration: BoxDecoration(color: const Color(0xFF8B0000).withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.menu_book, color: Color(0xFF8B0000), size: 22)),
        ...
        itemBuilder: (_, i) {
          ...
          return Card(margin: const EdgeInsets.only(bottom: 8), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), child: InkWell(
            ...
            child: Padding(padding: const EdgeInsets.all(14), child: Row(children: [
              Container(width: 6, height: 6, decoration: const BoxDecoration(color: Color(0xFF8B0000), shape: BoxShape.circle)),
